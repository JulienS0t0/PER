...
void multiplier_bloc(const void *bloc1, const void *bloc2, void *resultat, 
        ...
        for (int j = 0; j < BLOCK_SIZE && (debut_j + j) < taille; j++) {
            if (is_float) {
                ...
                for (int k = 0; k < taille; k++) {
                    somme += b1[(debut_i + i) * taille_totale + k] * 
                             b2[k * taille_totale + debut_j + j];
                }
                res[(debut_i + i) * taille_totale + debut_j + j] = somme;
            } else {
                int *b1 = (int*)bloc1;
                int *b2 = (int*)bloc2;
...
void multiplier_matrices_grandes(const void *data1, const void *data2, void *resultat,
                                 int taille, int is_float) {
    for (int i = 0; i < taille; i += BLOCK_SIZE) {
        for (int j = 0; j < taille; j += BLOCK_SIZE) {
            multiplier_bloc(data1, data2, resultat, i, j, taille, taille, is_float);
        }
    }
}
